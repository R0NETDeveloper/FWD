namespace Custom.InputAccel.UimScript
{
    ...
    class ScriptPolicyRightTransferForm : UimScriptDocument
    {
        ...
        public void FormLoad(IUimDataEntryFormContext form)
        {
            try
            {
                ...
                // 
                m.setDropdownProvince(form, listProvince, "ddl_assignee_address_province");  //ddl_Province ??
            }
            catch (Exception e)
            {
        ...
        public void ExitControl(IUimFormControlContext controlContext)
        {
            try
            {
                ...
                //case 1 
                if (controlContext.ControlName == "ddl_assignee_address_province") //ddl_Province ??
                {
                    //set Value Province
                    m.parseValueDropdown(controlContext, controlContext.ParentForm, "ddl_assignee_address_province", 2);//ddl_Province ??
                    // Fliter Zipcode Post Code

                    string value_postCode = controlContext.ParentForm.UimDataContext.FindFieldDataContext("assignee_address_provinceCode").ValueAsString; //Province ??  เพื่อเอา Code ไป search Zipcode
                    ...
                    if (!string.IsNullOrEmpty(value_postCode))
                    {
                        ...
                        m.setDropdownZipcode(controlContext.ParentForm, listZipcode, "ddl_assignee_address_zipCode");// ddl_zipcode ??
                    }



                }
                if (controlContext.ControlName == "ddl_assignee_address_zipCode") // ddl_zipcode ??
                {
                    m.parseValueDropdown(controlContext, controlContext.ParentForm, "ddl_assignee_address_zipCode", 1); // ddl_zipcode ??
                }
            }
            catch
            (Exception e)
